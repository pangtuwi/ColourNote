...
class ActivityRecords {
    ...
    static let instance = ActivityRecords()
    ...
    func createTable() {
        do {
            ...
            print("Unable to create table in ActivityRecords")
        }
    } //createTable
    
    
    func activityExists (searchId : Int) -> Bool {
      //  var result : Bool = false
       ...
       /* concurrentDBQueue.async(flags: .barrier) { [weak self] in
            ...
            do {
                for _ in try self.db!.prepare(self.activities.filter(self.activityId == searchId)) {
                    return true
                }
            } catch {
                print("search failed in ActivityRecords.activityExists")
            }
           // result = true
    ...
    func getActivities() -> [Activity] {
        var activities = [Activity]()
            ...
            do {
                for activity in try self.db!.prepare(self.activities) {
                ...
                print("Select failed in ActivityRecords.GetActivities() ")
            }
